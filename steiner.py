...
def steiner(t, k, v):
    ...
    # We want to pick a max_val so that the expected value of blocks is seleected.
    vmax = - log(total_blocks / block_size - 1)
    problem = CBPSO(dimensions, fitness, lambda x: len(x) == solution_size, vmax=vmax)

    status, solution, _ = problem.run()
...
if __name__ == '__main__':
    ...
    finished, solution = steiner(t, k, v)
